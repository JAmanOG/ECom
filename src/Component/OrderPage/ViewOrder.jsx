...
const ViewOrder = ({ orderId }) => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    if (!id || !newStatus) {
      console.error("Invalid order ID or status:", id, newStatus);
      return;
    }

    try {
      await updateOrderpage(id, newStatus);
      window.location.reload();
    } catch (error) {
      console.error("Error updating order status:", error);
      alert("There was an issue updating the order status. Please try again.");
  ...
  useEffect(() => {
    const fetchOrderDetailsFromDB = async (orderId) => {
      try {
        const updateStatus = await getUpdateOrderPage(orderId);
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden fade-in">
        <div className="p-6 bg-gradient-to-r from-purple-600 to-blue-500 text-white text-center">
          ...
          <p className="text-lg mt-2">
            Order #{parsedOrderData[0]?.orderId || "N/A"}
          </p>
        </div>
        ...
        <section aria-labelledby="order-heading" className="p-6">
            ...
            {parsedOrderData[0]?.items?.length > 0 ? (
              parsedOrderData[0].items.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-start mt-4 space-x-4 fade-in"
                  >
                    <img
              ...
              <div className="fade-in">
                ...
                <dd className="mt-1 text-sm text-gray-700">
                  <p>
                    ...
                    {parsedOrderData.paymentDetails}
                  </p>
                </dd>
              </div>
              <div className="fade-in">
          ...
          <div className="mt-8">
            ...
            <dl className="mt-4 fade-in">
              ...
              <div className="flex justify-between mt-3 border-t border-gray-200 pt-3">
                ...
                <dd className="text-md font-semibold text-gray-800">
                  ₹{parsedOrderData[0].totalPrice}
                </dd>
              </div>
            </dl>
          </div>
          {orderStatus.status === "Delivered" ? (
            <div className="mt-8 flex justify-between items-center fade-in">
              ...
              <InvoiceComponent
                orderId={parsedOrderData[0]?.orderId || "N/A"}
              />
            </div>
            ...
            <div className="mt-8 flex justify-between items-center fade-in">
              <button
                className="hover-button text-sm font-medium text-red-600 hover:text-red-500 focus:outline-none"
                onClick={() =>
                  handleStatusChange(parsedOrderData[0]?.orderId, "Canceled")
                }
              >
                Cancel Order
              </button>
